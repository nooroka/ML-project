...
#name = "PB_PF18073_1.fasta.fasta"
    ...
    for record in AlignIO.read(handle, "fasta") :
        record_dict[record.description].append(str(record.seq))
        text.append((record.description,record.seq))
    handle.close()
    ...
    for e in range(len(text3)):
        ...
        if kl%4 == 0:
            ...
            print(arr.shape)
            print(arr.dtype)
            #np.save("{}_{}.npy".format(name2[-1],strn),arr)
